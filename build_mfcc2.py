...
def initialize():
    env = lmdb.open("mfcc3",map_size=10000000000);
    return env;
...
word_list=[]
filename="/home/yang/speech/tag333.pkl"
if os.path.exists(filename):
	with open("tag333.pkl", 'rb') as f:
		...
		for (dirpath, dirnames, filenames) in os.walk(rootdir):
			...
			for filename in filenames:
				if filename.endswith('.wav') or filename.endswith('.WAV'):
					...
					key=string_n[7].replace(".wav","").replace(".WAV","")
					print("key:",key)
